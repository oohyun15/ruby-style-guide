...
* 메서드 호출 시 인자들 사이에 괄호를 사용하세요. 단, 인자를 제공하지 않는 메서드의 경우엔 괄호를 생략하세요.
  ...
  - 신택틱 슈거(syntactic sugar)를 통한 호출일 때 (예시로 `1 + 1`은 `+` 메서드를 호출하고, `foo[bar]`
    는 `[]` 메서드를 호출합니다.)
...
* `ensure` 블록에서 반환(return)하는 걸 피하세요. 만약 메서드 내 `ensure` 블록에서 명시적으로
  반환한다면 어떠한 예외들이 발생하더라도 정상적으로 반환하게 되고, 전혀 예외가 발생하지 않았더라도
...
* `Exception` 클래스를 사용한 예외 처리를 피하세요.
...
* 더 빠르고 전문화된 대안이 있는 경우 `String#gsub` 사용을 피하세요.
